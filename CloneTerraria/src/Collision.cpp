...

{
	...
	//float move[2] = { oldVelocity[0] * deltaTime + 0.5f * (velocity[0] - oldVelocity[0]) * deltaTime, oldVelocity[1] * deltaTime + 0.5f * (velocity[1] - oldVelocity[1]) * deltaTime };
	int wallVertices[4];
	int floorVertices[4];
	int edgeVertices[4];
	float a = objectVertices4f[0];
	float b = objectVertices4f[1];
	float c = objectVertices4f[2];
	float d = objectVertices4f[3];
...
}
{
	...
	if (velocity[0] > 0 && velocity[1] > 0)
	{
		...
		memoryDefender(floorVertices, 4);
		memoryDefender(edgeVertices, 4);
		unsigned int behavior = TwoDirectionCheck(hitbox, wallVertices, floorVertices, edgeVertices, objectVertices4f, 0, 3, 2, 1, transform, velocity, ceilHit, rightWallHit);
	}
	else if (velocity[0] < 0 && velocity[1] < 0)
	{
		int wallVertices[4];
